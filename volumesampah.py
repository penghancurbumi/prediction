...
st.subheader("📊 Diagram Batang Prediksi Total Sampah")

# Gabungkan data historis dan prediksi
tahun_pred_all['Total_Sampah_Pred'] = y_lin_future
bar_data = pd.merge(data_tahun, tahun_pred_all, on='Tahun', how='outer').sort_values('Tahun')
bar_data['Total_Sampah'] = bar_data['Total_Sampah'].fillna(bar_data['Total_Sampah_Pred'])

# Bar Chart
fig_bar, ax_bar = plt.subplots(figsize=(12, 6))
ax_bar.bar(bar_data['Tahun'], bar_data['Total_Sampah'], color='skyblue')
ax_bar.set_xlabel('Tahun')
ax_bar.set_ylabel('Total Sampah (ton)')
ax_bar.set_title('Diagram Batang Total Sampah Kota Sukabumi (Historis & Prediksi)')
